...
Ao criar um link simbólico, há dois casos, tanto a origem quanto o destino estão dentro de [.filename]#${PREFIX}#. Nesse caso, use `${RLN}`. No outro caso, se um ou ambos os caminhos estiverem fora de [.filename]#${PREFIX}# use `${LN} -s` e apenas prefixe [.filename]#${STAGEDIR}# para o caminho de destino.

[[staging-ex1]]
.Criar Links Simbólicos Relativos, Dentro de [.filename]#${PREFIX}#
[example]
====
...
[[staging-ex2]]
.Criar Links Simbólicos Absolutos, Fora de [.filename]#${PREFIX}#
[example]
====
Ao criar um link simbólico fora de [.filename]#${PREFIX}#, a origem não deve conter [.filename]#${STAGEDIR}#, o destino no entanto, deve:
...
Existem vários JDKs na coleção de ports, de vários fornecedores e em várias versões. Se o port precisar usar uma versão específica, especifique-a usando a variável `JAVA_VERSION`. A versão mais atual é package:java/openjdk8[], package:java/openjdk6[] e package:java/openjdk7[] também estão disponíveis.
...
Se uma versão específica do Lua for necessária, as instruções sobre como selecioná-la são fornecidas no <<uses-lua,`USES=lua`>> parte do <<uses>>.
...
|`LUA_SUBDIR`
|O diretório sob [.filename]#${PREFIX}/bin#, [.filename]#${PREFIX}/share# e [.filename]#${PREFIX}/lib# no qual o Lua é instalado
...
|`LUA_MODLIBDIR`
|O diretório no qual as bibliotecas dos módulos Lua ([.filename]#.so#) são instalados

|`LUA_MODSHAREDIR`
|O diretório no qual os módulos Lua ([.filename]#.lua#) são instalados
