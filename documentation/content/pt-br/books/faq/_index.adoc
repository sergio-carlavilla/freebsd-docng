...
authors: 
releaseinfo: "$FreeBSD: head/pt_BR.ISO8859-1/books/faq/book.xml 54408 2020-08-05 22:06:30Z dbaio $" 
trademarks: [] 
---
...
Sim. A documentação está disponível em vários formatos diferentes e esquemas de compressão no site FTP do FreeBSD, no diretório https://download.freebsd.org/ftp/doc/[/pub/FreeBSD/doc/].
...
|`rtf`
|Rich Text Format da Microsoft(TM)
...
[NOTE]
====
Números de página não são atualizados automaticamente ao carregar o formato Rich Text no Word. Pressione kbd:[Ctrl+A], kbd:[Ctrl+End], kbd:[F9] após carregar o documento, para atualizar os números das páginas.
====
...
+ 
Portanto, a versão PostScript(TM) do Handbook, compactada usando `bzip2` será armazenada em um arquivo chamado [.filename]#book.ps.bz2# no diretório [.filename]#handbook/#.
...
* Use o Subversion se for necessário a aplicação de patches customizados na árvore de ports. Consulte a seção link:{handbook}#svn[ Usando o Subversion ] para obter detalhes.
