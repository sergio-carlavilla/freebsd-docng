...
Para começar a configurar um KDC, adicione estas linhas ao arquivo [.filename]#/etc/rc.conf#:
...
Em seguida, inicie os serviços do KDC executando `service kdc start` e `service kadmind start`. Embora não haja daemons kerberizados em execução neste momento, é possível confirmar que o KDC está funcionando obtendo um ticket para o principal que acabou de ser criado:
...
* https://www.h5l.org/[Página Web do Kerberos Heimdal]
